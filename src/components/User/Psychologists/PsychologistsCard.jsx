...
function PsychologistsCard() {
  ...
  return (
    <div className="flex items-center flex-col gap-5 h-screen overflow-y-scroll custom-scrollbar">
      <Typography className="font-serif" variant="h3" color="white">
      ...
      {psychologists &&
        psychologists.map((psychologist, index) => (
          <div
            key={index}
            className="flex md:flex-row flex-col shadow-lg border-t bg-gray-100 bg-opacity-90 rounded-lg md:w-1/2 w-3/4 p-3 gap-3"
          >
            <div className="flex flex-col items-center lg:w-1/4">
              <img
                className="h-40 w-40 rounded-lg object-cover shadow-xl border-2"
                src={
                  psychologist.counselor_details.profile_image &&
                  psychologist.counselor_details.profile_image
                }
            ...
            <div className="flex  flex-col lg:w-3/4 mt-4 lg:items-start items-center  gap-2">
              <Typography className="font-serif font-semibold text-lg text-blue-800">
                Dr.{psychologist?.counselor_details?.first_name}{" "}
                {psychologist?.counselor_details?.last_name}
              </Typography>
              ...
              <Link to={`/psychologist-profile/?counselor=${psychologist.id}`}>
