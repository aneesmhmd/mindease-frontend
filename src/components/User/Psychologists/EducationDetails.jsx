...
function EducationDetails({ counselorId }) {
  ...
  const getEducations = async () => {
    await getCounselorEducation(counselorId)
      .then((res) => {
            ...
            <div className="flex flex-col mt-3 justify-start md:items-start md:ms-10 items-center gap-1 lg:w-3/4 w-full">
              <div className="flex flex-col mb-2 w-full">
                <div className="flex flex-row gap-2">
                  ...
                  <span className="text-blue-900 md:text-lg  font-serif">
                    {education?.qualification}{" "}
                    <span className="text-base">({education?.year})</span>
                  </span>
                </div>
                <h1 className="ms-6 text-sm font-sans">
                  {education?.university}, {education?.state},{" "}
                  {education.country}
                </h1>
              </div>
            </div>
