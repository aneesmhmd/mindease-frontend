...
import { getCounselorEducation, getCounselorProfile } from '../../../services/userApi';
...
function ViewProfile() {
    ...
    const getProfile = async () => {
        await getCounselorProfile(counselorId).then((res) => {
            setProfile(res.data)
            ...
            <div className='flex md:flex-row flex-col shadow-lg border-t bg-gray-100 bg-opacity-90 rounded-lg md:w-1/2 w-3/4 p-3 gap-3 mt-5'>
                ...
                <div className='flex flex-col items-center lg:w-1/4'>
                    ...
                    <img
                        ...
                        src={profile?.counselor_details?.profile_image && profile?.counselor_details?.profile_image}
                ...
                <div className='flex  flex-col lg:w-3/4 mt-4 lg:items-start items-center  gap-2'>
                    <Typography className="font-serif font-semibold text-lg text-blue-800">
                        Dr.{profile?.counselor_details?.first_name} {profile?.counselor_details?.last_name}
                    </Typography>

                    <div className='flex flex-row gap-2'>
                        <TagIcon strokeWidth={2}
                            className="h-4 w-4 mt-0.5" /><Typography className='text-sm font-sans'>
                            Top profile in {profile.state}
                        </Typography>
                    </div>
                    ...
                    <Link>
                        <Button size='sm' className='py-1 w-32 mt-2' color='blue-gray'>Book Slot</Button>
                    </Link>
